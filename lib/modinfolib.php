...
class course_modinfo {
     ...
     * @param boolean $usecache use cached section info if exists
     ...
     */
    public static function build_course_cache(\stdClass $course, bool $partialrebuild = true): \stdClass {
        if (empty($course->id)) {
     ...
     */
    protected static function inner_build_course_cache($course, \core\lock\lock $lock, $partialrebuild=true) {
        global $DB, $CFG;
 ...
 *                use course_invalidate_module_cache() to invalidate mod cache.
 *                use course_invalidate_section_cache() to invalidate section cache.
 *
 * @return void
 */
function rebuild_course_cache(int $courseid=0, bool $clearonly=false, bool $partialrebuild=true): void {
